...
void  WaveletMatrix::n_range_intersect_aux(size_t *x_start,size_t *x_end,size_t n_ranges)
{
...
void WaveletMatrix::n_range_intersect(uint lev, size_t * x_start, size_t *x_end,uint sym, size_t start, size_t end,size_t n_ranges,
...
void WaveletMatrix::range_report(uint lev, size_t x_start, size_t x_end,uint sym, size_t start, size_t end,vector <uint> &result)
{
	...
	if (lev < height)
	{
		...
		{
			start_new = bs->rank1(start_new-1);
			x_start_left = x_start - start_new+1;
		}
		else
		{
			x_start_left = 0;
