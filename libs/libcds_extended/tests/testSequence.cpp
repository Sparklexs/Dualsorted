...
void testSequence(Array & a, Sequence & bs) {
	...
	for(size_t i=0;i<a.getLength();i++) {
		...
		size_t r = 0;
		if (seq->access(i, r) != a[i]) {
			cerr << "ERROR ACCESS-RANK" << endl;
			cerr << "Wrong access" << endl;
			if (r != count[a[i]])
				cerr << "Wrong rank" << endl;
			exit(-1);
		}
		if (r != count[a[i]]) {
			cerr << "Wrong rank for ACCESS-RANK" << endl;
			exit(-1);
		}
...
int main(int argc, char ** argv) {
  ...
  WaveletTreeNoptrs wt1(a, new BitSequenceBuilderRG(20), mapper);
  cout << "bs.size() = " << wt1.getSize() << endl;
  testSequence(a, wt1);

  cout << "Test 2 : Wavelet matrix" << endl;
  WaveletMatrix wt2(a, new BitSequenceBuilderRG(20), mapper);
  cout << "bs.size() = " << wt2.getSize() << endl;
  testSequence(a, wt2);

  cout << "Test 3: Wavelet tree with pointers" << endl;
  WaveletTree wt3(a,new wt_coder_binary(a, mapper),new BitSequenceBuilderRG(20), mapper);
