 ...
 */
inline void lookupNext(uint *buffer, uint *ppos, uint *nn, uint N, uint *pprevvalue, 
            	uint gol_bits, tFastGolomb *golombRiceTables, uint searchedVal) {	
                       	
	uint index,pos,n,nCodes,i,nBitsDec,sumVal,prevvalue;
	nCodes=0;nBitsDec=0;
	pos = *ppos;  n=*nn;  prevvalue = *pprevvalue;
...
inline uint rice_encode2(uint *buf,uint pos,uint val)
{
	return rice_encode(buf,pos,val,10);
}
/*****************************************************************************/
inline uint rice_size(uint val, uint nbits)
{
